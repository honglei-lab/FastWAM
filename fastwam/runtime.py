...
from .common_base import (initialize_from_checkpoint, seed_initialization,
                          validate_options, verify_and_record_identity)
...
def run_training(cfg: DictConfig):
    ...
    init_sha256 = cfg.get("init_checkpoint_sha256")
    validate_options(init_checkpoint, init_sha256, cfg.get("resume"))
    if init_checkpoint:
        output = Path(cfg.output_dir).resolve()
        source = Path(init_checkpoint).resolve()
        if output == source.parent or output in source.parents:
            raise ValueError("Common-base run output must not contain its source checkpoint")
        if (output / "checkpoints").exists() or list(output.glob("common-base-init-rank*.json")):
            raise FileExistsError("Use a new output directory for common-base initialization")
    init_seed = cfg.get("init_seed")
    seed_initialization(cfg.seed if init_seed is None else init_seed)
    setup_logging(
    ...
    initialization = (initialize_from_checkpoint(model, init_checkpoint, init_sha256)
                      if init_checkpoint else None)
    expected_model = cfg.get("init_expected_model_sha256")
    if initialization and expected_model and initialization["initial_model"]["sha256"] != expected_model:
        raise ValueError("Initial full model differs from the sealed common base")
    ...
    if initialization is not None:
        initialization["initialization_seed"] = int(cfg.seed if init_seed is None else init_seed)
        initialization["training_seed"] = int(cfg.seed)
        verify_and_record_identity(initialization, cfg.output_dir, trainer)
    trainer.train()
